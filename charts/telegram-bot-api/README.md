...
The workload uses a `StatefulSet`. When `persistence.enabled=true`, each replica gets its own PVC via `volumeClaimTemplates` (`data-<statefulset>-<ordinal>`).
...
| `persistence.enabled` | bool | `true` | Enables per-replica persistent data volume via StatefulSet `volumeClaimTemplates`. |
